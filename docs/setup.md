...
ブックを初めて開いたとき、保存先がすべて空であれば、保存先の親フォルダをたずねるダイアログが表示されます（既定 `C:\KnowledgeMgr`）。入力すると、その下に `data` / `formats` / `ui` / `log` / `backup` などの保存先が自動で設定されます。あとから変えるときは、管理.xlsm の **M-10「格納先設定」画面** で値を直し、**[保存]** ボタンを押します。各設定値の意味は[設定値の変更](settings.md)を参照してください。
