項目値層ユーティリティ層種別標準モジュール役割によるサムネ画像配置化済行数行配置先で挿入標準モジュール、でプロパティ→オブジェクト名をに変更してから、コードペインに貼り付けます。ソースコード（コピペ可）型を要求するに対し、経由だとがでにされる。経由なら型チェックで数値リテラル渡しが通る。第引数第引数サムネサイズ詳細画像最大サイズ名削除再配置用セル画像未配置画像未配置関連呼び出すなし呼び出される---
...
VBE で `挿入 > 標準モジュール`、F4 でプロパティ → `(オブジェクト名)` を `modImageRender` に変更してから、コードペインに貼り付けます。
