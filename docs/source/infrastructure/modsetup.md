...
| 層 | インストーラ層 |
| 種別 | 標準モジュール (.bas) |
| 役割 | 14 シート + 29 ボタンを 1 回実行で自動生成 (ADR-0008) |
| 行数 | 682 行 |
...
Private Sub SetButtonCaptionAndAction(ByVal shp As Shape, _
    ...
    ' 两方失敗したら Debug.Print で警告 (ロガー依存を作らずに告知)
    If Not oleSet Then
...
ErrHandler:
    ...
    If Not wsErr Is Nothing Then
        ...
        wsErr.Cells(13, 1).Value = "[警告] ヘッダ初期化失敗: " & Err.Description
    End If
End Sub
```
...
Private Sub PlaceOneButton(ByVal ws As Worksheet, _
    ...
    Dim shp As Shape
    Set shp = ws.Shapes.AddFormControl(XL_BUTTON_CONTROL, leftPt, topPt, widthPt, heightPt)
    shp.Name = macroName

    Call SetButtonCaptionAndAction(shp, caption, macroName)
End Sub
