...
Private Function IsLengthShortened(ByVal oldLen As String, ByVal newLen As String) As Boolean
    ...
    If Len(oldLen) = 0 Or Len(newLen) = 0 Then
        ...
        Exit Function
    End If
    IsLengthShortened = (CLng(newLen) < CLng(oldLen))
    Exit Function
ErrHandler:
    ' Q7 規約 Y：Resume Next 直後の Err check
    If Err.Number <> 0 Then
        If Not m_logger Is Nothing Then
            m_logger.LogError "clsFieldMigrator", "IsLengthShortened", Err.Description, "", "LOG-MIG-LEN-ERR"
        End If
        Err.Clear
    End If
    IsLengthShortened = False
...
' Dropdown 値削除判定
Private Function IsDropdownOptionRemoved(ByVal oldOpts As String, ByVal newOpts As String) As Boolean
    On Error GoTo ErrHandler
    If Len(oldOpts) = 0 Then
        IsDropdownOptionRemoved = False
        Exit Function
    End If
    If Len(newOpts) = 0 Then
        IsDropdownOptionRemoved = True
        Exit Function
    End If

    Dim oldArr() As String
    Dim newArr() As String
    oldArr = Split(oldOpts, ",")
    newArr = Split(newOpts, ",")
    ...
    For i = LBound(oldArr) To UBound(oldArr)
        ...
        For j = LBound(newArr) To UBound(newArr)
            If Trim$(oldArr(i)) = Trim$(newArr(j)) Then
                found = True
                Exit For
            End If
        Next j
...
ErrHandler:
    If Not m_logger Is Nothing Then
        m_logger.LogError "clsFieldMigrator", "IsDropdownOptionRemoved", Err.Description, "", "LOG-MIG-DROPDN-ERR"
    End If
    IsDropdownOptionRemoved = False
End Function
```
