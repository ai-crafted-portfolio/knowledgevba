...
Public Function ValidateInput() As Boolean
    On Error GoTo ErrHandler
    ' Screen-level input precondition check (M-08 search).
    ...
    ' Search filter fields are optional by design, so an initialised
    ' spec validates True; an uninitialised screen fails the precondition.
    If m_spec Is Nothing Then
        Call modScreenRender.LogScreenTrace("clsSearchScreen", "ValidateInput", "spec not initialised", "VALIDATE-WARN-WW-208")
        ValidateInput = False
        Exit Function
    End If
    ...
    For Each fld In m_spec.Fields
        If fld.Required And Len(fld.InputAddr) > 0 Then
            If Len(Trim$(GetInputValue(fld.InputAddr))) = 0 Then
                Call modScreenRender.LogScreenTrace("clsSearchScreen", "ValidateInput", "required field empty: " & fld.Label, "VALIDATE-WARN-WW-208")
                ValidateInput = False
                Exit Function
            End If
        End If
...
ErrHandler:
    Call modScreenRender.LogScreenError("clsSearchScreen", "ValidateInput", "validate", Err.Number, Err.Description, "VALIDATE-WARN-WW-208")
    ValidateInput = False
End Function
