...
description: modUILoader.bas 縺ｮ繧ｽ繝ｼ繧ｹ繧ｳ繝ｼ繝会ｼ医さ繝斐・逕ｨ・・---
...
## 繝輔ぃ繧､繝ｫ縺ｨ縺励※菫晏ｭ・
繝｡繝｢蟶ｳ・医∪縺溘・莉ｻ諢上・繝・く繧ｹ繝医お繝・ぅ繧ｿ・峨↓荳九・繧ｽ繝ｼ繧ｹ繧ｳ繝ｼ繝牙・譁・ｒ雋ｼ繧贋ｻ倥￠縲・*`modUILoader.bas`** 縺ｨ縺・≧蜷榊燕縺ｧ `installer\vba_modules\common\` 驟堺ｸ九↓菫晏ｭ倥＠縺ｦ縺上□縺輔＞縲よ枚蟄励さ繝ｼ繝峨・ ANSI・・hift-JIS・峨∵隼陦後・ CRLF 縺ｫ縺励※縺上□縺輔＞縲・
---
...
Private Sub ApplyDropdownValidation(ByVal r As Range, ByVal source As String)
    ...
    ElseIf Left(source, 7) = "format:" Then
        ' format:list -> dynamic load all format ids from format_dir.
        ' format:<id>:fields:<no> remains deferred (cross-module I/O).
        If LCase(Mid(source, 8)) = "list" Then
            Dim formats As Collection
            On Error Resume Next
            Set formats = modFormatLoader.ListAllFormats
            On Error GoTo 0
            ...
            For fi = 1 To formats.Count
                ...
                listCsv = listCsv & CStr(formats(fi))
            Next fi
        Else
            Debug.Print "[D-1497] modUILoader.ApplyDropdownValidation EXIT-OK"  ' [ADR-0100]
            Exit Sub
