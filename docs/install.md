...
インストール後に保存先や動作設定を変えたいときは、**管理.xlsm の M-10「格納先設定」画面**で値を直し、**[保存]** ボタンを押します（ブックが上書き保存されます）。各設定値の意味は[設定値の変更](settings.md)を参照してください。
...
REM    2026-06-07: M-04 / M-14 retire 済み。シート構成は
REM    M-02 / M-03 / M-12 / M-10 / M-11 / LOG の 6 シート
...
4. シートタブで **M-10 格納先設定** を開き、保存先フォルダのパスが入っていることを確認します。値を変えたときは **[保存]** ボタンを押すとブックに保存されます
