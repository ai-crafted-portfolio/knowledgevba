...
現行 v2.3 の仕様をまとめたリファレンスです。構成やしくみの説明は[全体アーキテクチャ](architecture.md)を参照してください。
...
| M-05 | ナレッジ登録 | 検索.xlsm | ナレッジの新規登録 |
| M-06 | ナレッジ修正 | 検索.xlsm | 既存ナレッジの修正 |
...
| ブック | 含まれる画面 | 起動画面 |
|---|---|---|
| 検索.xlsm | M-05 / M-06 / M-07 / M-08 / M-09 | M-08 ナレッジ検索 |
...
動作設定は、各ブック内の **「格納先設定」シート** に 9 項目（6 つの保存先フォルダ＋3 つの動作設定）としてまとめられています。値のセルを書き換えてブックを保存すると変更が反映されます（以前のテキスト設定ファイルは v2.3 で廃止）。主な設定項目は次のとおりです。

| 設定 | 内容 |
|---|---|
| data_dir / format_dir / ui_dir / log_dir / backup_dir / config_dir | ナレッジ・フォーマット・画面定義・ログ・バックアップ・設定の格納先フォルダ |
...
設定値の意味と変更方法は[設定・カスタマイズ](screen_customization_guide_v2.md)で詳しく説明します。
...
すべてのモジュールの一覧・役割・ソースコードは[モジュール一覧](modules/index.md)に掲載しています。
