...
![完了後のメイン画面](images/installer/06_after_install.png)

ここまで来たら導入は完了です。あとはボタンを押して使い始めるだけ。

> 一番上の **「初回セットアップ」** を 1 回押すと、必要なシート（格納先設定 / 設定 など）
> が自動で作られます。最初の 1 回だけ押してください。

---

## 困ったとき（FAQ）

### Q1. 「access to the VBA Project is not trusted」と出て止まる

STEP 1 のチェックボックスが入っていません。Excel に戻って STEP 1 をもう一度やり直してください。

### Q2. 黒い窓が一瞬出てすぐ消える

ダブルクリックで bat を開いただけだと、エラーメッセージが見えずに閉じます。
**bat に Excel ブックをドラッグ＆ドロップ** で起動してください
（bat 単体起動は使い方として想定していません）。

### Q3. もう一度 STEP 3 を実行しても大丈夫？

大丈夫です。何度でも安全に実行できます。同じ `.xlsm` を再度ドラッグすれば
最新の VBA に上書きされます。**シート上のデータ（あなたが登録したナレッジ）は
消えません**。
