...
変更は、管理.xlsm の **M-10「格納先設定」画面** で値を直し、**[保存]** ボタンを押すだけです。設定項目（`data_dir` などの保存先 6 項目と、`debugLevel` / `logRotationRows` / `uiSchemaFailMode` の 3 項目）の意味と値は、[設定値の変更](../settings.md) のページで詳しく説明しています。
