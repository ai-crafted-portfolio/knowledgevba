...
ナレッジ管理ツールは、フォーマット定義や画面の見た目を **外部のテキストファイル** で管理しています。ファイルを書き換えるだけで、入力項目や画面の見た目を変更できます。保存先フォルダやログなどの動作設定は、各ブック内の **「格納先設定」シート** で管理します（[設定値の変更](../settings.md) を参照）。
...
| 種類 | 置き場所 | 役割 |
|---|---|---|
| 動作設定（格納先設定シート） | 各ブック内の「格納先設定」シート | 保存先フォルダや動作を決める（[設定値の変更](../settings.md)） |
...
変更は、管理.xlsm の **M-10「格納先設定」画面** で値を直し、**Ctrl+S** で保存するだけです。設定項目（`data_dir` などの保存先 6 項目と、`debugLevel` / `logRotationRows` / `uiSchemaFailMode` の 3 項目）の意味と値は、[設定値の変更](../settings.md) のページで詳しく説明しています。
